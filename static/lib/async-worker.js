(function() {
	...
	var runInWorker = async (arglist,definition,args=[],timeLimit=3000) => {
		...
		for(let i=0;i<WebAPIList.length;i++) {
			if(typeof(window[WebAPIList[i]]) != 'undefined') {
				secCheckList.push(WebAPIList[i]);
			}
		}

		var hd = 'for(let i=0;i<'+secCheckList.length+';i++){';
		hd += 'let t = ' + JSON.stringify(secCheckList) + '[i];';
		hd += 'if(t != "postMessage")self[t]=undefined';
		hd += '}';
		var code = 'var __target_func = (';
		for(let i=0;i<arglist.length;i++) {
			if(i>0) code += ',';
			code += arglist[i];
		}
		...
		return await new Promise((resolve,reject) => {
			...
			worker.postMessage('Start working, you son of b****!');
