...
(() => {
	var scroller_sqrt = [];
	window.scroll_init = function() {
		for(var i=0;i<=110;i++) {
			scroller_sqrt[i] = Math.pow(i,0.72);
		}
	}
	var scroller_interval_T=-1;
	window.scrollto = function(v,s,d=(G.is_wap ? 16 : 28)) {
		if(scroller_interval_T != -1) {
			clearInterval(scroller_interval_T);
			scroller_interval_T = -1;
		}
		...
		var I=0;
		scroller_interval_T = setInterval(function(){
	...
	var scroller_interval_L=-1;
	window.scrollto_left = function(v,s,d=(G.is_wap ? 16 : 28)) {
		if(scroller_interval_L != -1) {
			clearInterval(scroller_interval_L);
			scroller_interval_L = -1;
		}
		...
		var E=(v-old)/scroller_sqrt[d];
		var I=0;
		scroller_interval_L = setInterval(function(){
			E = (v-ele.scrollLeft)/scroller_sqrt[d-I];
			ele.scrollLeft+=E;
			I++;
			if(I>=d || (ele.scrollLeft-v)/E >=0) {
				clearInterval(scroller_interval_L);
				scroller_interval_L = -1;
			}
		},G.is_wap ? 15 : 10);
...
$('document').ready(function(){
	...
	window.tick_func = function(){
		if(frame_count % 3 != 0) return; // 每 3 帧一次
		if(!isNormal) return;
	...
	registerFrameFunc(save_progress);
...
function playPreInit() {
	...
	$('.menu-curr-display > .addition-cmt').append($('<span class="txmp-tag tag-cyan-g">'+LNG('list.tag.author')+escapeXml(listMeta[curr_idx]['LA'])+' | '+escapeXml(listMeta[curr_idx]['MA'])+'</span>'));
