...
var scroller_sqrt = [];
function scrollto(v,s,d=<?php echo $w ? '12' : '28' ?>)
{
	...
	$('.lrc-interval-item').each(function(idx){
		...
		if(lrcline.hasClass('lrc-interval-item')) {
			...
			if(lrcline.hasClass('lrc-active')) {
				...
				lrcline.css({'font-family':'宋体'});
			}
			else {
				if(thistime==0) txt='&gt; 回到开头 &lt;';
				else txt='- - - - - - -';
...
$('document').ready(function(){
	...
	A.onerror=function(){
		...
		modal_alert('出问题了','音频加载失败<br>错误代码：'+A.error.code);
		...
		curr=nxt;
		localStorage['txmp-procsave-'+list[nxt]]=0;
	...
	var update_f=function(){
		...
		if(A.src.substr(A.src.length-4) == '.url') { //震惊，是TXT url
			...
			//modal_alert('test','[调试] 发起了新的TXT加载');
			$.ajax({
				async:true,
				timeout:20000,
				dataType:'text',
				...
				error: function(e) {
					...
					//modal_alert('出问题了','TXT地址获取失败');
				},
				success: function(e) {
					A_is_txt_loading = false;
					if(e.length < 7) { // 长度小于'http://'，肯定咕
						//modal_alert('出问题了','错误数据：<br>'+e);
					}
					else {
		...
		localStorage['txmp-procsave-'+song_id]=A.currentTime;
	...
	//从RojExplorer > v447d 来的链接（准备保存歌单）
	(function(){

		var ref=document.referrer;

		//引荐地址示例：https://fake.com/path/to/rojexplorer/index.php?appcallback/go&url=txmp&filename=/音乐集/TXMP歌单.oexe
		...
		if(isFmSave) {
			document.title=titleformat.replace('%{list_name}',data['meta']['N']+' < '+basename(localStorage['fm-save-loc-'+fmRandId]));
		...
		if(isCloudSave) {
			document.title=titleformat.replace('%{list_name}',data['meta']['N']+' < ' + cloudData['title']);
		}
...
function trackSwitch() {
	...
	//modal_alert("提示","功能未完成");
	//return;
	if(obj.getAttribute("data-back")!="yes") {
		modal_alert("提示","本歌曲不包含人声消减的音频");
	}
	else {
		var T=A.currentTime;
		var H=A.paused;
...
function addPlaytimes(id) {
	var num_id = 'txmp-playtimes-'+id;
	var date_id = 'txmp-playdate-'+id;
...
function playPreInit() {
	if(isCloudSave) {
		if(G_username != cloudUser) {
			$('.edit-label').html('贺来改一改');
			if(G_username == '') {
	...
	if(isList) {
		...
		RP_next = function(){
			addPlaytimes('finish-'+song_id);
			localStorage['txmp-procsave-'+list[curr]]=0;
...
function playInit(){
	...
	if(localStorage['txmp-procsave-'+song_id]<A.duration-1) {
		A.currentTime=localStorage['txmp-procsave-'+song_id];
	}
...
function switchContent(e,dst_song,flag=false) {
	...
	if(isFmSave) document.title=titleformat.replace('%{list_name}',data['meta']['N']+' < '+basename(localStorage['fm-save-loc-'+fmRandId]));
...
function changeTo(dst_song,flag=false) {
	var al=modal_loading("稍等","正在切换歌曲");
	...
	setTimeout(function(){
		...
		if(!Cache[dst_song]) $.ajax({
			...
			close_modal(al);
			//modal_alert("Test","[调试] 使用预读的数据进行切换");
...
async function listEdit() {
	if(!await modal_confirm_p('编辑歌单','确定离开播放器，编辑歌单？')) return;
	if(!isCloudSave) {
		var txt=
		...
		if(G_username == cloudUser) {
			...
			var str = '';
			var wnd = modal_loading('请稍候','正在为您<b>贺</b>此歌单');
			if(!isCsv) str = JSON.stringify(cloudData);
			else str = await fetch_raw_data();
			if(str === undefined) {
				await modal_alert_p('出问题了','<b>贺</b>是不道德的');
				return;
			}
			$.ajax({
				async: true,
...
async function listDownload() {
	if(!await modal_confirm_p('生成离线播放器','确定生成？\n生成一次可能时间较长，不可下载的歌曲将自动忽略。')) return;
	var txt=
		home+'offline-player?list=';
	for(var i=0;i<ordlist.length;i++) {
		if(i!=0) txt+='|';
		txt+=ordlist[i];
	}
...
async function setVolume() {
	var txt=await modal_prompt_p('更改相对音量','输入音量（0到1之间的小数）。<br>注意保持音量健康，否则可能损伤听力。',A.volume);
	if(!txt) return;
	localStorage['txmp-volume']=txt;
	A.volume=txt;
}

async function setZoom() {
	var txt=await modal_prompt_p('歌词缩放','输入缩放倍率',localStorage['txmp-zoom']);
	if(!txt) return;
	localStorage['txmp-zoom']=txt;
...
function rmenu_hide() {
	...
	$('#right-menu').css({marginRight:'calc(<?php echo $w?'-100%':'-480px' ?> - 8px)'});
}
