...
function parseCmpLyric($u,$parseTags = true,$debug = false) {
	...
	if(_CT('compiled_cache') && !kuwo_classify_id($u) && $parseTags == true && $debug == false) {
		...
		if(file_exists($cache_fn)) {
			if(filemtime($cache_fn) >= filemtime(FILES . $u . '/lyric.txt') && filemtime($cache_fn) >= filemtime(__FILE__)) {
	...
	try {
		...
		foreach($txt as $lineid => $v)
		{
			...
			if($v && (strlen($v)<2 || substr($v,0,2)!="//") && (strlen($v)<2 || substr($v,0,2)!="##") && (strlen($v)<1 || substr($v,0,1)!="!"))
			{
				...
				if($v[0]=='[')
				{
					if($v[strlen($v)-1]!=']')
					{
						$ft = '第 '.($lineid+1).' 行：此行开头有左中括号，但是末尾没有右中括号。这是无论如何也不被允许的（包括在注释中）。';
						$msg .= '<strong>致命错误：</strong>'.$ft."\n";
						$e = errorJson(
					...
					foreach($vi as $k2=>$v2)
					{
						...
						else
						{
							...
							{
								$ft = '第 '.($lineid+1).' 行：段开头的类型不支持。可接受的值为：'.concat_arguments($allow_heading,0);
								$msg .= '<strong>致命错误：</strong>'.$ft."\n";
								$e = errorJson(
									'Fatal','WrongHeading',
									$ft
		...
		{
			if(!$k && $v['type']!="Para") {
				$ft = '第 '.$v['__line'].' 行：文档第一个非空行并不是段开头。';
				$msg .= '<strong>致命错误：</strong>'.$ft."\n";
				$e = errorJson(
			...
			if($v['type']=="Para")
			{
				...
				if($v['cmd'] == 'Info') {
					if($hasInfo) {
						$msg .= '<strong>警告：</strong>第 '.$v['__line'].' 行：歌词文件中出现了多个 Info 标签。注意只有最早出现的会被正确处理。'."\n";
					}
					$hasInfo = true;
				}
		...
		if(!$hasInfo) {
			$msg .= '<strong>警告：</strong>歌词文件中没有 Info 标签，这会导致“歌曲标题缺失”的致命错误。'."\n";
		}
		...
		$w_template = "不合规范的做法可能导致未来版本的新特性无法正确处理此歌曲。\n";
		if(
			strstr($meta['meta']['S'],'&') ||
			strstr($meta['meta']['S'],'|') ||
			strstr($meta['meta']['S'],',') ||
			...
			$msg .= '<strong>不合规范：</strong>第 '.findKVInArray($xinfo,"type","S")['__line'].' 行：多个歌手间只应该用符号“/”来分割。'.$w_template;
		}
		...
		if(
			...
			$msg .= '<strong>不合规范：</strong>第 '.findKVInArray($xinfo,"type","S")['__line'].' 行：多个作词者间只应该用符号“/”来分割。'.$w_template;
		}
		...
		if(
			...
			$msg .= '<strong>不合规范：</strong>第 '.findKVInArray($xinfo,"type","S")['__line'].' 行：多个作曲者间只应该用符号“/”来分割。'.$w_template;
		}
		...
		if(!$meta['meta']['N']) {
			$ft = '第 '.findKVInArray($am,"type","Info")['__line'].' 行：歌曲标题缺失。这是无论如何也不被允许的。';
			$msg .= '<strong>致命错误：</strong>'.$ft."\n";
			$e = errorJson(
				'Fatal','NoSongTitle',
				$ft
		...
		foreach($am as $v)
		{
			if($v['type']=='Reuse')
			{
				$pid++;
				if($v['arg'][0][0]!='@') {
					$ft = '第 '.$v['__line'].' 行：Reuse标签指定的id不以@开头';
					$msg .= '<strong>致命错误：</strong>'.$ft."\n";
				...
				if(!isset($reuses[$v['arg'][0]])) {
					$ft = '第 '.$v['__line'].' 行：Reuse标签指定的id在上文没有出现过';
					$msg .= '<strong>致命错误：</strong>'.$ft."\n";
				...
				foreach($meta['lyrics'][$pid]['in'] as $f=>$h)
				{
					...
					if($lastTime == $meta['lyrics'][$pid]['in'][$f]['ts']) {
						$msg .= '<strong>警告：</strong>第 '.$v['__line'].' 行：这行歌词和上一句拥有完全相同的切入时间。如果这句歌词是翻译或歌曲信息，请将其改为注释。R'."\n";
					}
			...
			if($v['type']=='Similar') {
				...
				$acstart=0;
				if($v['arg'][0][0]!='@') {
					$ft = '第 '.$v['__line'].' 行：Similar标签指定的id不以@开头';
					$msg .= '<strong>致命错误：</strong>'.$ft."\n";
				...
				if(!isset($reuses[$v['arg'][$acstart]])) {
					$ft = '第 '.$v['__line'].' 行：Similar标签指定的id在上文没有出现过';
					$msg .= '<strong>致命错误：</strong>'.$ft."\n";
				...
				if(count($v['cmds'])>count($meta['lyrics'][$reuseitem]['in'])) {
					$ft = '第 '.$v['__line'].' 行：Similar的段长度超过所引用的。';
					$msg .= '<strong>致命错误：</strong>'.$ft."\n";
				...
				foreach($v['cmds'] as $h)
				{
					if($h['type']=='L')
					{
						...
						if($lastTime == $meta['lyrics'][$pid]['in'][$k]['ts']) {
							$msg .= '<strong>警告：</strong>第 '.$h['__line'].' 行：这行歌词和上一句拥有完全相同的切入时间。如果这句歌词是翻译或歌曲信息，请将其改为注释。S'."\n";
						}
						...
						if($meta['lyrics'][$pid]['in'][$k]['ts'] < 1610612736 && str_included($str,[
							...
							$msg .= '<strong>不合规范：</strong>第 '.$h['__line'].' 行：歌词行中出现了 /:$—[] 以外的标点符号。如果这是一句歌词翻译或歌曲信息，请将其改为注释。'."\n";
						}
			...
			if($v['type']=='Para' || $v['type']=='Hidden')
			{
				...
				foreach($v['cmds'] as $h)
				{
					if($h['type']=='L')
					{
						...
						$str = concat_arguments($h['arg'],1);
						if($lastTime == $meta['lyrics'][$pid]['in'][$k]['ts']) {
							$msg .= '<strong>警告：</strong>第 '.$h['__line'].' 行：这行歌词和上一句拥有完全相同的切入时间。如果这句歌词是翻译或歌曲信息，请将其改为注释。P'."\n";
						}
						if($meta['lyrics'][$pid]['in'][$k]['ts'] < 1610612736) $lastTime = $meta['lyrics'][$pid]['in'][$k]['ts'];
						if($meta['lyrics'][$pid]['in'][$k]['ts'] < 1610612736 && str_included($str,[
							',','，','.','。','"','“','”','‘','’','?','？','!','！','：','；',';','~','`',
		...
		if(_CT('compiled_cache') && !kuwo_classify_id($u) && $parseTags == true && $debug == false) {
			$cache_fn = FILES . $u . '/compiled.json';
			file_put_contents($cache_fn,json_encode($meta,JSON_UNESCAPED_SLASHES+JSON_UNESCAPED_UNICODE));
		}

		if(!$debug) return json_encode($meta,JSON_UNESCAPED_SLASHES+JSON_UNESCAPED_UNICODE);
		else {
			return [
				"message" => $msg,
				"source" => $srccode,
...
function buildMinifiedLrc($data) {
	...
	// return json_encode($contmap,JSON_PRETTY_PRINT+JSON_UNESCAPED_UNICODE);
	return $ret;
}
