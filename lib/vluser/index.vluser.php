...
class VlUserSyscall {
	...
	function allplay_obj($menu,$name='____') {
		if($name == '____') {
			$name = LNG('list.playlist_name');
		}
		$obj = [];
		$obj['public'] = true;
		$obj['title'] = $name;
	...
	function dataExist($cate,$name) {
		if(substr($name,strlen($name) - 5) != '.json') {
			return false;
		}
		return $this -> data($cate,preSubstr($name,'.'),false);
	}
	...
	function fetchData($cate,$name) {
		...
		return $this -> data($cate,preSubstr($name,'.'),true);
	}
};
