...
?>
<script>document.title='<?php LNGe('pass.title') ?> - <?php echo htmlspecial2(_CT('app_name_title')) ?>';</script>
<div class="txmp-page-full">
    ...
    <p><form method="post">
        ...
        <input type="submit" class="am-btn am-btn-danger <?php if(uauth_ip_cnt($_SERVER['REMOTE_ADDR']) >= _CT('ip_reg_limit')) echo 'am-disabled' ?>" value="<?php LNGe('pass.field.change') ?>">
    </form></p>
    <hr>
    <p><?php LNGe('pass.remove.caption') ?></p>
    <p><form method="post">
        <input type="hidden" name="csrf-token-name" value="<?php echo $GLOBALS['sess'] ?>">
        <input type="hidden" name="csrf-token-value" value="<?php echo $GLOBALS['token'] ?>">
