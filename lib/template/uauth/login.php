...
if(!isset($_POST['isSubmit']));
	...
	if($status=='passwrong') {
		redirectToNote(LNGk('uauth.login.wrongpass'));
		exit;
	}
	else if($status=='loggedin') {
		redirectToNote(LNGk('uauth.login.loggedin'));
		exit;
	}
	else if($status=='ban') {
		redirectToNote(LNGk('uauth.login.banned'));
		exit;
	}
	else if($status=='nxuser') {
		redirectToNote(LNGk('uauth.login.nxuser'));
		exit;
	}
	{redirectToGet();exit;}
}
	...
	if(!_CT('can_register')) {
		redirectToNote(LNGk('uauth.ureg.notallow'));
		exit;
	}
	if(!$_POST['pass']) {
		redirectToNote(LNGk('uauth.ureg.nopass'));
		exit;
	}
	if($_POST['pass'] !== $_POST['passagain']) {
		redirectToNote(LNGk('uauth.ureg.wrongpass'));
		exit;
	}
	if(!$_POST['name'] || strlen($_POST['name'])<3 || strlen($_POST['name'])>14) {
		redirectToNote(LNGk('uauth.ureg.namelen'));
		exit;
	}
	$status = uauth_register($_POST['name'],$_POST['pass'],false,'none');
	if($status == 'success') {
		redirectToNote(LNGk('uauth.ureg.success'));
		exit;
	}
	else if($status == 'exist') {
		redirectToNote(LNGk('uauth.ureg.occupied'));
		exit;
	}
	else if($status == 'illegal') {
		redirectToNote(LNGk('uauth.ureg.illegal'));
		exit;
	}
	else if($status == 'loggedin') {
		redirectToNote(LNGk('uauth.ureg.loggedin'));
		exit;
	}
	else if($status == 'limit') {
		redirectToNote(LNGk('uauth.ureg.limited'));
		exit;
	}
	redirectToNote(LNGk('uauth.ureg.uke',$status));
	exit;
}
